...
module.exports = {
    ...
    async run(bot, message, args, db) {
        ...
        if ((await message.guild.members.fetchMe()).roles.highest.comparePositionTo(member.roles.highest) <= 0) return interaction.reply("Le robot ne peux pas warn ce membre !")

        try { await user.send(`${message.user.tag} Vous a warn du serveur ${message.guild.name} pour la raison : \`${reason}\``) } catch (err) {}
